...
class BackyardFlyer(Drone):
    ...
    def __init__(self, connection):
        ...
        self.all_waypoints = [[15.0, 0.0, 3.0],
                              [15.0, 15.0, 3.0],
                              [0.0, 15.0, 3.0],
                              [0.0, 0.0, 3.0]]
    ...
    def local_position_callback(self):
        ...
        if self.flight_state == States.WAYPOINT:
            point = self.calculate_box()[self.current_position]
            if ((abs(self.local_position[0] - point[0]) < 0.1) and
               (abs(self.local_position[1] - point[1]) < 0.1) and
               (abs(altitude - point[2]) < 0.1)):
    ...
    def velocity_callback(self):
        ...
        if ((self.flight_state == States.LANDING) and
           (self.global_position[2] - self.global_position[2] < 0.1) and
    ...
    def waypoint_transition(self):
        ...
        points = self.calculate_box()
        if self.current_position == len(points):
            self.takeoff_transition()
        else:
            point = points[self.current_position]
            self.cmd_position(point[0], point[1], point[2], 0)
